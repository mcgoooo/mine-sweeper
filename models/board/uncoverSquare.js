...
export default ({ game, rowIndex, squareIndex}) => {
...
const recurseEmptySquares = ({ game, rowIndex, squareIndex }) => {
  ...
  recursablePositionsOffset.forEach((position)=>{
    ...
    if (adjacentSquare && adjacentSquare.uncovered !== true) recurseEmptySquares({
      game,
      rowIndex: row,
      squareIndex: column
    })
